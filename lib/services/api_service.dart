...
class ApiService {
  ...
  Future<Map<String, dynamic>> getProfile() async {
    ...
    print('Access Token: $accessToken'); // Print the access token for debugging
    ...
    try {
      ...
      if (response.statusCode == 200 && responseBody['status'] == 'success') {
        print(
            'Profile Data: $responseBody'); // Print the profile data if successful
        return responseBody;
      } else {
        print(
            'Error Loading Profile: ${responseBody['message']}'); // Print error message if present
        throw Exception('Failed to load profile');
      }
    } catch (e) {
      print('Exception occurred: $e'); // Print the exception if an error occurs
      return {};
    }
  }
